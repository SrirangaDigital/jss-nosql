...
class Model {
	...
	public function getThumbnailPath($id){
		...
		$artefactPath = PHY_ARCHIVES_URL . $id;

		$leaves = glob(PHY_ARCHIVES_URL . $id . '/thumbs/*.JPG');

		$firstLeaf = array_shift($leaves);
		return ($firstLeaf) ? str_replace(PHY_ARCHIVES_URL, ARCHIVES_URL, $firstLeaf) : STOCK_IMAGE_URL . 'default-image.png';
	}
	...
	public function syncArtefactJsonToDB($id){
		...
		$jsonFile = PHY_ARCHIVES_URL . $id . '/index.json';
